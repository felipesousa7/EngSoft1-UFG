<?php
    ...
    if( (!isset($_SESSION['idCluster'])) && (!isset($_POST['idCluster']))){
        ...
        if( $stmt = mysqli_prepare($conn, $sqlUpdate) ){
            mysqli_stmt_bind_param($stmt, "sssssssssssssssssssssssssss",$nome,$descricao,$img1,$img2,$img3,$img4,$img5,$img6,$img7,$img8,$t1,$t2,$t3,$t4,$t5,$t6,$t7,$t8,$leg1,$leg2,$leg3,$leg4,$leg5,$leg6,$leg7,$leg8,$_SESSION['idCluster']);
            if(mysqli_stmt_execute($stmt)){
    ...
    for ($controle = 0; $controle < count($arquivo['name']); $controle++){
        ...
        if ( strstr ( '.jpg;.jpeg;.png', $extensao ) ) {
            ...
            $destino = '../public/imagens/cluster/' . $novoNome;
            // tenta mover o arquivo para o destino
            if ( @move_uploaded_file ( $arquivo_tmp, $destino ) ) {
                session_start();
                $id = $_SESSION['id'];
                $sql = "UPDATE cluster SET $colunaNome = ? WHERE id = ?";
                if( $stmt = mysqli_prepare($conn, $sql) ){
                    mysqli_stmt_bind_param($stmt, "ss",$destino,$idCluster);
                    if(mysqli_stmt_execute($stmt)){
